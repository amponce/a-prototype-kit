...
const getGitHubRepoContent = async (
  ...
  try {
    ...
    const token = operation === 'fetchTemplate' ? templateToken || userToken : userToken;

    // Log which token is being used (for debugging and notifications)
    const isUsingSystemToken =
      operation === 'fetchTemplate' && templateToken && (!userToken || token === templateToken);
