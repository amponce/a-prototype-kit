  ...
  useEffect(() => {
    if (modelList.length > 0) {
      ...
      if (filtered.length > 0 && !filtered.some((model) => model.name === selectedModel)) {
        console.log(
          `Selected model ${selectedModel} not found in provider ${selectedProvider}. Resetting to ${filtered[0].name}`,
        );
  ...
  return (
    <div className="space-y-6">
      ...
      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filteredModels.map((model) => (
                ...
                >
                  ...
                  {model.name === selectedModel && <div className="i-ph:check-circle-fill text-blue-500 w-5 h-5" />}
                </div>
              ))}
            </div>
          )}
        ...
        {currentProvider && (
          <div className="p-4 bg-bolt-elements-background-depth-2 rounded-lg border border-bolt-elements-borderColor">
            <h5 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">{selectedProvider} API Key</h5>
