...
export const updateSelectedModel = (model: string) => {
  selectedModelStore.set(model);
  localStorage.setItem('selectedModel', model);
};

// Function to update selected provider
export const updateSelectedProvider = (provider: string) => {
  ...
  try {
    ...
    if (savedApiKeys) {
      try {
        ...
        console.log('Initializing with saved API keys from cookies, keys for providers:', Object.keys(parsedKeys));
        selectedApiKeysStore.set(parsedKeys);
      } catch (error) {
        console.error('Error parsing saved API keys:', error);
      }
    } else {
      console.log('No saved API keys in cookies');
    }
  } catch (error) {
